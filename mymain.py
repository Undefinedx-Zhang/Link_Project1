...
from mylink import Ui_Dialog as u

class Node():   #链表中的一个结点
    def __init__(self, id=None, name=None, birthday=None, sex=None, photo=None, nation=None, index=None):
        self._id = id
        self._name = name
...
class MyLink(QMainWindow, u):
    ...
    def insert_item(self):  #插入信息
        if self.namebox.text() == '' or self.idbox.text() =='' or self.nationbox.text()=='':
        ...
        node = Node(id, name, birthday, sex, self.pic_path, nation,  index)
    ...
    def change_item(self):      #修改信息
        iid = self.lineEdit_4.text()
        node = friends_link.find(iid)
        if self.name_2.text() == '' or  self.nation_2.text()=='' or self.sexx.text == '' or self.birthdayy.text =='':
            QMessageBox.about(self, "提示", "信息未填写完整！")
            return
